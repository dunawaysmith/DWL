...
{   
    ...
    onAwake() 
    {
        ...
        var physicsCollider: ColliderComponent = this.getSceneObject().getComponent("ColliderComponent")
        if(isNull(physicsCollider))
        {
            physicsCollider = this.getSceneObject().createComponent("ColliderComponent")
        }
    ...
    public response(item: EventResponseType, eventType: EventType)
    {
        switch(item.eventResponseType)
        {
            case 0:
                //set state
                var setStateDelayedCallback = this.createEvent("DelayedCallbackEvent")
    ...
    private initIterateThroughChildren(itemsArray: EventResponseType[])
    {
        for(let i = 0; i < itemsArray.length; i++)
        {
            ...
            {
                var children = item.sceneObject.children
    ...
    {     
        var iterationIndex = this.setIterationIndex(eventType)
        
        var childrenList = item.sceneObject.children
        var currentIndexItem = childrenList[iterationIndex % childrenList.length]
        currentIndexItem.enabled = false

        var nextIndexItem = childrenList[(iterationIndex + 1) % childrenList.length]
    ...
    private playTransformAnimations(item: EventResponseType, eventType: EventType)
    {
        ...
        {
            var tween: Tween
            let animationInfo = item.animations[i]
    ...
    {   
        var animationPlayer = item.sceneObject.getComponent("AnimationPlayer")
        ...
        var animationClips: AnimationClip[] = animationPlayer.clips

        if(item.customAnimationData.iterateThroughAllClips == true)
        {
            var iterationIndex = this.setIterationIndex(eventType)

            var clipName: string = animationClips[iterationIndex % animationClips.length].name
        ...
        else
        {
            ...
            var clip = animationPlayer.getClip(item.customAnimationData.animationName)
            if(isNull(clip))
            {
    ...
    {
        var renderMeshV = item.sceneObject.getComponent("RenderMeshVisual")
        ...
        {
            var material_copy = renderMeshV.mainMaterial.clone()
    ...
    private changeColor(item: EventResponseType)
    {
        ...
        {
            var material_copy = meshVisual.mainMaterial.clone()
    ...
    {
        var blendShapeData = item.blendShapeData
        var renderMeshVisual = item.sceneObject.getComponent("RenderMeshVisual")
    ...
    {
        var audioComponent = item.sceneObject.getComponent("AudioComponent")
        ...
        else
        {
            switch(item.audioControl.audioControlBehavior)
            {
                case 0:
                    //play
                    var playAudioDelayedCallback = this.createEvent("DelayedCallbackEvent")
    ...
    {
        var image = item.sceneObject.getComponent("Image")
        if(!isNull(image))
        {
            var videoTextureProvider: VideoTextureProvider = image.mainMaterial.mainPass.baseTex.control as VideoTextureProvider
            if(!isNull(videoTextureProvider))
            {
                var playVideoTextureDelayedCallback = this.createEvent("DelayedCallbackEvent")
