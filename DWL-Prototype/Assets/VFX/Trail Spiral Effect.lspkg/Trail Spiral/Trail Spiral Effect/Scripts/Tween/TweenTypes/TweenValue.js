...
script.value = getRawByName("start");
script.start = null;
script.end = null;
...
function setStart(start) {
    ...
    script.start = getValueFromRaw(start);
}

// Manually set end value
function setEnd(end) {
    script.manualEnd = true;
    script.end = getValueFromRaw(end);
}
...
function updateToEnd() {
    updateValue((script.loopType == 3) ? script.start : script.end);
}
...
function setupTween() {
    ...
    if (!script.manualStart) {
        ...
        script.start = getValueFromRaw(startRaw);

    }

    if (!script.manualEnd) {
        var endRaw = getRawByName("end");
        script.end = getValueFromRaw(endRaw);
    }

    var startValue = script.start;
    var endValue = script.end;
...
function setupTweenBackwards() {
    var startValue = (script.loopType == 3) ? script.start : script.end;

    var endValue = (script.loopType == 3) ? script.end : script.start;
...
function updateValue(value) {
    if (script.propertiesCount <= 1) {
        script.value = (script.propertiesCount == 0) ? Math.floor(value.a) : value.a;
    } else {
        for (var i = 0; i < script.propertiesCount; i++) {
            script.value[properties[i]] = value[properties[i]];
        }
    }
    if (script.onUpdateCallback != null) {
        script.onUpdateCallback(script.value);
    }
}
...
function getCallbackFunction() {
    ...
    switch (script.callbackType) {
        ...
        case (1):
            ...
            return function(v) {
                script.propTargetScript[script.propName] = v;
            };
        ...
        case (2):
            ...
            return function(v) {
                script.funcTargetScript[script.funcName](v);
            };
        ...
        case (3):
            ...
            if (!typeIsMatching(script.value, pass[script.materialScriptName])) {
        ...
        case (4):
            ...
            if (!typeIsMatching(script.value, vfxAsset.properties[script.vfxScriptName])) {
